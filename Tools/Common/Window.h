 ...
 * make sure that the global variable is defined in the same source
 * file where you use this class.
 * For example, when using setXxxHandler() in A.cpp to register
 * a function, which is implemented in B.cpp. Then you should put
...
#error unsupported platform
    ...
    void isRunning() {
        if (tWnd.joinable())
            throw ustr_error(ustr("Cannot set handler when the window is running"));
    }
...
#error unsupported platform
     ...
     */
    void alert(const string_t& str, const string_t& title, MessageType type = INFO);
