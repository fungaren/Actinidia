...
void DX::DeviceResources::CreateDeviceIndependentResources()
{
	...
	winrt::check_hresult(
		CoCreateInstance(
			...
			__uuidof(m_wicFactory), m_wicFactory.put_void()
			)
		);
}
...
void DX::DeviceResources::CreateDeviceResources() 
{
	...
	HRESULT hr = D3D11CreateDevice(
		...
		device.put(),				// 返回创建的 Direct3D 设备。
		&m_d3dFeatureLevel,			// 返回所创建设备的功能级别。
		context.put()				// 返回设备的即时上下文。
		);
...
void DX::DeviceResources::CreateWindowSizeDependentResources() 
{
	...
	else
	{
		...
		m_swapChainPanel.Dispatcher().RunAsync(CoreDispatcherPriority::High, [=]()
		{
			...
			winrt::com_ptr<ISwapChainPanelNative> panelNative = nullptr;
			winrt::check_hresult(
				winrt::get_unknown(m_swapChainPanel)->QueryInterface(__uuidof(panelNative), panelNative.put_void())
				);
	...
	winrt::com_ptr<IDXGISwapChain2> spSwapChain2 = nullptr;
	winrt::check_hresult(
		m_swapChain.try_as(spSwapChain2)
		);
	...
	winrt::com_ptr<IDXGISurface2> dxgiBackBuffer = nullptr;
	winrt::check_hresult(
...
void DX::DeviceResources::ValidateDevice()
{
	...
	winrt::com_ptr<IDXGIFactory2> deviceFactory = nullptr;
	winrt::check_hresult(deviceAdapter->GetParent(__uuidof(deviceFactory), deviceFactory.put_void()));
	...
	winrt::check_hresult(CreateDXGIFactory1(__uuidof(currentFactory), currentFactory.put_void()));
